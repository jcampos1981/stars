...
print("="*70)
print(" MALLORN TDE Classification - Fine-Tuned Prediction Pipeline")
print("="*70)
...
detailed_path = f'predictions_finetuned_with_probabilities.csv'
...
submission_path = f'submission_finetuned_{timestamp}.csv'
...
print(f"  - Fine-tuned model (max_depth=5, n_estimators=300)")
print(f"  - Feature selection ({len(selected_features)} features)")
print(f"  - Conservative threshold ({threshold:.4f})")
print(f"  - CV F1: {model_data['cv_f1_mean']:.4f} ± {model_data['cv_f1_std']:.4f}")
print("\nExpected improvement over robust model:")
print("  - Slightly deeper trees for better fit")
print("  - More estimators for stability")
print("  - CV-optimized threshold (conservative)")
print("="*70)
