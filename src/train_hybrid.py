"""
Hybrid training: Base features + Top 7 color features
...
from feature_engineer_hybrid import HybridFeatureEngineer

print("="*70)
print(" MALLORN TDE Classification - Hybrid Training Pipeline")
print("="*70)
print("Strategy: Base features + Top 7 color features")
print("Features: ~99 total (92 base + 7 color)")
print("="*70)
...
# Initialize data loader and hybrid feature engineer
loader = DataLoader(data_dir)
engineer = HybridFeatureEngineer(apply_deextinction=True)
...
# Load or extract hybrid features
features_path = processed_dir / 'train_features_hybrid.csv'
if features_path.exists():
    print(f"\n[2/7] Loading existing hybrid features from {features_path}...")
    features_df = pd.read_csv(features_path)
    print(f"Loaded features for {len(features_df)} objects")
else:
    print(f"\n[2/7] Extracting hybrid features (base + top 7 color)...")
    ...
    print(f"Hybrid features saved to {features_path}")
...
# Check how many color features were selected
color_features_selected = [f for f in selected_features if 'color_' in f]
print(f"Color features selected: {len(color_features_selected)}")
if color_features_selected:
    print(f"Selected color features: {color_features_selected}")

X = pd.DataFrame(X_selected, columns=selected_features)

# === HYBRID MODEL (same params as fine-tuned) ===
print(f"\n[5/7] Training hybrid model...")
print("Using fine-tuned model parameters:")
print("  - max_depth: 5")
print("  - n_estimators: 300")
print("  - Strong regularization maintained")

model = xgb.XGBClassifier(
    max_depth=5,              # From fine-tuned
    learning_rate=0.05,
    n_estimators=300,         # From fine-tuned
    min_child_weight=5,
    gamma=0.2,
    subsample=0.7,
...
model_data = {
    ...
    'color_features': color_features_selected
}

model_path = models_dir / 'tde_classifier_hybrid.pkl'
...
features_path = models_dir / 'feature_names_hybrid.txt'
...
print("="*70)
print(f"\nFinal Results:")
print(f"  Model: XGBoost (Hybrid - Base + Top 7 Color Features)")
print(f"  CV F1 Score: {np.mean(cv_scores):.4f} ± {np.std(cv_scores):.4f}")
print(f"  Train F1 Score: {train_f1:.4f}")
print(f"  Total Features: {len(selected_features)}")
print(f"  Color Features: {len(color_features_selected)}")
print(f"  Conservative Threshold: {conservative_threshold:.4f}")
print(f"  Optimal Threshold: {optimal_threshold:.4f}")
print("\nStrategy:")
print("  - Base features from LightcurveFeatureEngineer")
print("  - Plus top 7 color features: u_g_peak, u_g_evolution, r_z_peak,")
print("    g_r_peak, g_r_mean, i_z_peak, i_z_evolution")
