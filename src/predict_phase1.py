"""
Generate predictions using Phase 1 model (base + temporal evolution features)
"""
...
print("="*70)
print(" MALLORN TDE Classification - Phase 1 Model Prediction")
print("="*70)
print("Model: Base features + Temporal Evolution Features")
print("="*70)
...
print(f"\n[1/5] Loading Phase 1 model from {model_path}...")
...
print(f"Phase 1 features: {model_data['n_phase1_features']}")
...
print(f"\nModel Info:")
print(f"  Strategy: Phase 1 (Base + Temporal Evolution Features)")
print(f"  CV F1: {model_data['cv_f1_mean']:.4f} ± {model_data['cv_f1_std']:.4f}")
print(f"  Threshold: {threshold:.4f}")
print(f"  Predicted TDEs: {n_tde} ({pct_tde:.2f}%)")
print(f"\nKey Phase 1 Features:")
if model_data['phase1_features']:
    for f in model_data['phase1_features'][:5]:
        print(f"  - {f}")
    if len(model_data['phase1_features']) > 5:
        print(f"  ... and {len(model_data['phase1_features']) - 5} more")
print(f"{'='*70}")
