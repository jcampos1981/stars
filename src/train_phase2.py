"""
Phase 2 Training: Temperature & Color Evolution Features
Train model with Phase 1 + Phase 2 features for improved TDE/AGN/SN separation
"""
...
print("="*70)
print(" MALLORN TDE Classification - Phase 2 Training")
print("="*70)
print("Strategy: Phase 1 + Temperature/Color Evolution Features")
print("Goal: Separate TDEs from AGN/SN using spectral evolution")
print("="*70)
...
# Load or extract Phase 2 features
features_path = processed_dir / 'train_features_phase2.csv'
if features_path.exists():
    print(f"\n[2/7] Loading existing Phase 2 features from {features_path}...")
    features_df = pd.read_csv(features_path)
    print(f"Loaded features for {len(features_df)} objects")
else:
    print(f"\n[2/7] Extracting Phase 2 features (Phase 1 + temperature/color)...")
    ...
    print(f"Phase 2 features saved to {features_path}")
...
# Check how many Phase 2 features were selected
phase2_features = [f for f in selected_features if any(keyword in f for keyword in [
    'color_', 'uv_optical', 'temp_proxy', 'avg_color', 'fraction_colors',
    'sed_', 'evolution_consistency'
])]

print(f"Phase 2 features selected: {len(phase2_features)}")
if len(phase2_features) > 0:
    print(f"Top Phase 2 features selected:")
    for f in phase2_features[:10]:
        print(f"  - {f}")

X = pd.DataFrame(X_selected, columns=selected_features)

# === PHASE 2 MODEL ===
print(f"\n[5/7] Training Phase 2 model...")
...
model_data = {
    'model': model,
    'model_name': 'XGBoost (Phase 2 - Temp/Color Evolution)',
    ...
    'n_phase2_features': len(phase2_features),
...
print(f"Phase 2 model saved to: {model_path}")

# Save selected features
features_path = models_dir / 'feature_names_phase2.txt'
...
print("="*70)
print(f"\nFinal Results:")
print(f"  Model: XGBoost (Phase 2 - Temperature & Color Evolution)")
...
print("\nPhase 2 Strategy:")
print("  - Phase 1 temporal evolution features")
print("  - Plus temperature & color evolution features:")
print("    * Color indices at early/peak/late epochs")
print("    * UV/optical ratio evolution")
print("    * Temperature cooling proxies (u-g, g-r)")
print("    * Multi-color evolution consistency")
print("    * SED shape characteristics")
print("    * Color variability (smooth for TDEs, erratic for AGN)")
...
print("\nExpected Impact:")
print("  - Target: Separate TDEs from AGN/SN using spectral evolution")
print("  - Goal: Test F1 0.54-0.57 (from Phase 1: 0.5105)")
print("="*70)
