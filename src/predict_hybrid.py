"""
Generate predictions using hybrid model (base + top 7 color features)
"""
...
print("="*70)
print(" MALLORN TDE Classification - Hybrid Model Prediction")
print("="*70)
print("Model: Base features + Top 7 color features")
print("="*70)
...
print(f"\n[1/5] Loading hybrid model from {model_path}...")
...
print(f"Color features: {len(model_data.get('color_features', []))}")
...
# Extract features
print(f"\n[3/5] Extracting hybrid features for test set...")
...
print(f"\nModel Info:")
print(f"  Strategy: Hybrid (Base + Top 7 Color Features)")
print(f"  CV F1: {model_data['cv_f1_mean']:.4f} ± {model_data['cv_f1_std']:.4f}")
print(f"  Threshold: {threshold:.4f}")
print(f"  Predicted TDEs: {n_tde} ({pct_tde:.2f}%)")
print(f"{'='*70}")
